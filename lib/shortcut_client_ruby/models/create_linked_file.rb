  ...
  class CreateLinkedFile
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shortcut::CreateLinkedFile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shortcut::CreateLinkedFile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('Object', ['google', 'url', 'dropbox', 'box', 'onedrive'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['google', 'url', 'dropbox', 'box', 'onedrive'])
    ...
    def hash
      [description, story_id, name, thumbnail_url, type, size, uploader_id, content_type, url].hash
    end
