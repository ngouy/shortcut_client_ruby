  ...
  class CreateStoryContents
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shortcut::CreateStoryContents` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shortcut::CreateStoryContents`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [description, entity_type, labels, story_type, linked_files, file_ids, name, epic_id, external_links, iteration_id, tasks, label_ids, group_id, workflow_state_id, follower_ids, owner_ids, estimate, files, project_id, linked_file_ids, deadline].hash
    end
