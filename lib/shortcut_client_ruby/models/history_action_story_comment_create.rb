...
module Shortcut
  ...
  class HistoryActionStoryCommentCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shortcut::HistoryActionStoryCommentCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shortcut::HistoryActionStoryCommentCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] app_url Value to be assigned
    def app_url=(app_url)
      if app_url.nil?
        fail ArgumentError, 'app_url cannot be nil'
      end

      if app_url.to_s.length > 2048
        fail ArgumentError, 'invalid value for "app_url", the character length must be smaller than or equal to 2048.'
      end

      pattern = Regexp.new(/^https?:\/\/.+$/)
      if app_url !~ pattern
        fail ArgumentError, "invalid value for \"app_url\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [id, entity_type, action, app_url, text, author_id].hash
    end
